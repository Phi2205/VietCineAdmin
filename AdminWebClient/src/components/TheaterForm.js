...
const TheaterForm = () => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`http://localhost:8080/api/admin/theaters/${id}`)
        .then((response) => {
          ...
          setTheater({
            ...
            city: data.city || "",
            theaterBrandId: data.theaterBrandId || null,
          });
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        setBrands(response.data); // Giả sử trả về mảng brand { id, name }
        console.log("Fetched brands:", response.data);
      })
      .catch((error) => {
        console.error("Error fetching brands:", error);
        toast.error("Không lấy được danh sách thương hiệu rạp");
      });
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    const theaterToSubmit = {
      ...
      city: theater.city,
      theaterBrandId: theater.theaterBrandId,
    };

    const request = id
      ? axios.put(
          `http://localhost:8080/api/admin/theaters/${id}`,
          theaterToSubmit
        )
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit}>
            <Stack spacing={2}>
              ...
              <FormControl fullWidth required>
                <InputLabel id="brand-label">Thương hiệu rạp</InputLabel>
                <Select
                  labelId="brand-label"
                  name="theaterBrandId"
                  value={theater.theaterBrandId}
                  onChange={handleChange}
              ...
              >
                <Button
                  ...
                  color="secondary"
                  startIcon={<ArrowBack />}
                  onClick={() => navigate("/theaterBrands")}
                >
                  Cancel
                </Button>
                <Button
