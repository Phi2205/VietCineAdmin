...
function AddTheaterBrandDialog({
  open,
  onClose,
  onAdd,
  onUpdate,
  ...
  useEffect(() => {
    if (open) {
      if (editingBrand) {
        setName(editingBrand.theaterBrandName || "");
        setLogo(null); // Giữ null để người dùng chọn file mới nếu muốn thay đổi
  ...
  const handleSubmit = (e) => {
    ...
    // Nếu thêm mới, logo là bắt buộc
    if (!logo && !editingBrand) {
      setLogoError(true);
      hasError = true;
    }
    ...
    >
      ...
      <DialogContent sx={{ py: 3 }}>
        ...
        <Typography sx={{ mb: 1, color: "#fff" }}>
          Logo thương hiệu {editingBrand ? "(có thể bỏ qua)" : "*"}
        </Typography>
...
const loadAllTheaters = async () => {
  ...
  const handleAddBrand = (newBrand) => {
    axios
      .post("http://localhost:8080/api/admin/theaterbrands", newBrand, {
  ...
  const handleDeleteConfirm = () => {
    if (theaterToDelete) {
      axios
        .delete(
          `http://localhost:8080/api/admin/theaters/${theaterToDelete.id}`
        )
        .then(() => {
          setTheaters((prev) =>
            prev.filter((theater) => theater.id !== theaterToDelete.id)
          );
  ...
  const editTheaterBrand = (newBrand) => {
    console.log("New brand ", newBrand);
    const idTheaterBrand = newBrand.get("id");

    newBrand.delete("id");
    console.log("New brand ", newBrand);
    console.log("ID brand ", idTheaterBrand);
    axios
      .put(
        `http://localhost:8080/api/admin/theaterbrands/${idTheaterBrand}`,
        newBrand,
        {
          // **KHÔNG tự set Content-Type để axios tự thêm boundary**
        }
  ...
  const totalScreens = theaters.reduce(
    (sum, theater) => sum + theater.totalScreens,
    0
  );
        ...
        <Box sx={{ p: 2 }}>
          ...
          >
            <Typography
              variant="h6"
              color="white"
              sx={{ fontWeight: "bold", mb: 2, fontSize: 24 }}
            >
              Hãng rạp chiếu phim
            </Typography>

            <Tooltip
              title={openUpdateTheaterBrand ? "Đóng chỉnh sửa" : "Chỉnh sửa"}
            >
              <IconButton
                onClick={updateTheaterBrand}
                color="primary"
                size="small"
              >
                {openUpdateTheaterBrand ? <CloseIcon /> : <EditIcon />}
              </IconButton>
            </Tooltip>
          </Box>
          ...
          <Card
            ...
            {theaterBrands.map((brand) => (
              <Card
                ...
                >
                  {openUpdateTheaterBrand && (
                    <IconButton
                      size="small"
                      sx={{
                        bgcolor: "rgba(25, 118, 210, 0.7)",
                        color: "#fff",
                        "&:hover": { bgcolor: "rgba(76, 175, 80, 0.7)" },
                      }}
                      onClick={() => {
                        setEditingTheaterBrand(brand);
                        setOpen(true);
                      }}
                    >
                  ...
                  {openUpdateTheaterBrand && (
                    <IconButton
                      size="small"
                      sx={{
                        bgcolor: "rgba(244, 67, 54, 0.7)", // red 500 with transparency
                        color: "#fff",
                        "&:hover": {
                          bgcolor: "rgba(244, 67, 54, 0.9)", // đậm hơn khi hover
                        },
                      }}
            ...
            <AddTheaterBrandDialog
              ...
              onUpdate={editTheaterBrand}
              editingBrand={editingTheaterBrand}
            />
          </Box>
        </Box>
        {/* Search Bar */}
                  ...
                  {filteredTheaters.map((theater) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Stack direction="row" spacing={1} alignItems="center">
                          <Tv
                            style={{ width: 20, height: 20, color: "#FF9100" }}
                          />
                          <Typography>
