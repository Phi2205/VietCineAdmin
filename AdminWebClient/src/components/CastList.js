...
const CastDialog = ({ open, onClose, onAdd, onUpdate, editingCast }) => {
  ...
  useEffect(() => {
    if (open) {
      if (editingCast) {
        setName(editingCast.name || "");
        setAvatar(null);
        setAvatarPreview(editingCast.avatar || null);
      } else {
        setName("");
        setAvatar(null);
...
const handleSubmit = async (e) => {
  ...
  // Kiểm tra avatar chỉ khi tạo mới và đảm bảo avatar là file hợp lệ
  if (!editingCast && (!avatar || !(avatar instanceof File) || !avatar.type.startsWith("image/"))) {
    setAvatarError(true);
    hasError = true;
  }
  ...
  if (editingCast && editingCast.id) {
    formData.append("id", editingCast.id);
  }
  ...
  try {
    ...
    toast.error(errorMessage, { toastId: "cast-error" });
  } finally {
    setLoading(false);
  }
};
  ...
  return (
    ...
    >
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", color: "#fff" }}>
        {editingCast ? "Cập nhật diễn viên" : "Thêm diễn viên"}
      ...
      <DialogContent sx={{ py: 3 }}>
        <TextField
          ...
          error={nameError}
          helperText={nameError ? "Vui lòng nhập tên diễn viên." : ""}
          sx={{ mb: 3, input: { color: "#fff" }, label: { color: "#aaa" } }}
          disabled={loading}
        />

        <Typography sx={{ mb: 1, color: "#fff" }}>
          Ảnh đại diện {editingCast ? "(có thể bỏ qua)" : "*"}
        </Typography>
      ...
      <DialogActions sx={{ px: 3, pb: 3 }}>
        ...
        <Button
          ...
          disabled={loading}
        >
          {loading ? <CircularProgress size={24} /> : editingCast ? "Cập nhật diễn viên" : "Thêm diễn viên"}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
const CastManagement = () => {
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      setFilteredCasts(casts.filter((cast) => cast));
    }
  }, [casts, searchTerm]);
  ...
  const fetchCasts = async () => {
    setLoading(true);
    try {
      const response = await axios.get("http://localhost:8080/api/admin/casts");
      const validCasts = response.data.filter(
        (cast) => cast && cast.id && cast.name
      );
      setCasts(validCasts);
      setFilteredCasts(validCasts);
    } catch (error) {
      console.error("Error fetching casts:", error);
      toast.error(error.response?.data?.message || "Lỗi khi tải dữ liệu diễn viên");
    } finally {
      setLoading(false);
    }
  };

  const handleAddCast = async (formData) => {
    try {
      const response = await axios.post("http://localhost:8080/api/admin/casts", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      if (response.status === 200 && response.data?.data?.id && response.data?.data?.name) {
        await fetchCasts();
        toast.success(response.data.message || "Thêm diễn viên thành công", {
          toastId: "add-cast",
        });
      } else {
        throw new Error(response.data?.message || "Dữ liệu diễn viên không hợp lệ");
      }
    } catch (error) {
      console.error("Error adding cast:", error);
      throw new Error(error.response?.data?.message || "Lỗi khi thêm diễn viên");
    }
  };

  const handleEditCast = async (formData) => {
    const idCast = formData.get("id");
    formData.delete("id");

    try {
      const response = await axios.put(
        `http://localhost:8080/api/admin/casts/${idCast}`,
        formData,
        { headers: { "Content-Type": "multipart/form-data" } }
      );
      ...
      throw new Error(error.response?.data?.error || "Lỗi khi cập nhật diễn viên");
    }
  };
          ...
          >
            <Box>
              ...
              <Typography variant="subtitle1" sx={{ mt: 0.5, color: "rgba(255,255,255,0.85)" }}>
                Quản lý tất cả diễn viên trong hệ thống
              </Typography>
            </Box>
            <Button
              variant="contained"
        ...
        {loading ? (
          ...
          <Fade in={!loading}>
            ...
            >
              ...
              <Typography variant="body1" color="text.secondary">
                Vui lòng điều chỉnh tìm kiếm hoặc thêm diễn viên mới
              </Typography>
            </Paper>
          </Fade>
        ) : (
                    ...
                    cast && cast.name ? (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Tooltip title="Chỉnh sửa diễn viên">
                            <IconButton
                              onClick={() => {
                                setEditingCast(cast);
                                setDialogOpen(true);
